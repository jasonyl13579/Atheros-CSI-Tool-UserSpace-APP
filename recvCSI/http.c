...
#define HTTP_JSON_POST "POST /%s HTTP/1.1\r\nHOST: %s:%d\r\nAccept: */*\r\n"\
...
static char *http_parse_result(const char*lpbuf)
{
    char *ptmp = NULL; 
    char *response = NULL;
    ptmp = (char*)strstr(lpbuf,"HTTP/1.0");
    if(!ptmp){
        printf("http/1.0 not faind\n");
        return NULL;
    }
    if(atoi(ptmp + 9)!=200){
        printf("result:\n%s\n",lpbuf);
...
char * http_post(const char *url,const char *post_str){
    ...
    sprintf(lpbuf,HTTP_JSON_POST,file,host_addr,port,strlen(post_str),post_str);
